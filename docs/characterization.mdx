...
**Fig. 1**: Integration of prototype device with a flow sensor on the output port (above), and sample flow rate and pressure waveforms from the prototype device at different flow rates (below). Flow rates are set by the flow regulator attached to the oxygen cylinder and are not controlled by the device.
...
**Fig. 2**: Effects of upstream tubing length, with 1 ft of upstream tubing (left grid of 6 waveform plots) and 2 ft of upstream tubing (right grid of 6 waveform plots). Waveforms were recorded with software simulated trigger.
